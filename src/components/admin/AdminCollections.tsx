...
const empty: Partial<Collection> = {
  ...
  image_url: "", images: [], primary_image_index: 0,
...
export default function AdminCollections() {
    ...
    try {
      for (const file of list) {
        ...
        if (file.size > 8 * 1024 * 1024) {
          toast.error(`${file.name}: over 8 MB, skipped.`);
          continue;
        }
      ...
      if (newUrls.length) {
        setEditing((cur) => {
          const prev = cur?.images ?? [];
          const merged = [...prev, ...newUrls];
          return {
            ...(cur ?? {}),
            images: merged,
            // if no legacy single image, set the first uploaded as image_url
            image_url: cur?.image_url || newUrls[0],
  ...
  const save = async () => {
    ...
    const primaryUrl = images[primaryIdx] || editing.image_url || null;
    const payload = {
      ...
      images,
      primary_image_index: primaryIdx,
  ...
  // Image gallery operations inside the editor
  ...
  const setPrimary = (idx: number) => {
    setEditing((cur) => cur ? { ...cur, primary_image_index: idx } : cur);
  };
              ...
              <div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    {(editing.images ?? []).map((url, idx) => {
                      ...
                      return (
                        <div
                          key={url + idx}
                          className={`relative group border-2 transition ${
                            isPrimary ? "border-gold" : "border-border"
                          }`}
                        >
                          <img src={url} alt={`photo ${idx + 1}`} className="w-full aspect-square object-cover" />
                          {isPrimary && (
                ...
                >
                  {uploading ? (
                    ...
                    <><Upload className="h-3.5 w-3.5" /> Upload photos (multiple)</>
                  )}
                </button>
                <p className="text-[11px] text-muted-foreground leading-relaxed mt-2">
                  Pick several at once. JPG / PNG / WebP · max 8 MB each. Hover a photo to reorder, remove, or set as primary (the first image on the saree page).
                </p>
              </div>
